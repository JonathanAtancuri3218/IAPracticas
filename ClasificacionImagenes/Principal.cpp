...
list <ImagenTest> read () {
    ...
    read.open ("iids_test.txt", ios::out | ios::in);
...
int main(int argc, char *argv[]){
    ...
    const char *PATH_TEST ={ "imagenes/test/"};
    // Initialize array of pointer
    const char *PATHS[6] = { "imagenes/train/accordion/", "imagenes/train/binocular/",
                             "imagenes/train/pyramid/", "imagenes/train/snoopy/",
							 "imagenes/train/stop_sign/","imagenes/train/wheelchair/" };
    //Categorias de clasificacion
    const char *Categorias[6] = {"accordion", "binocular", "pyramid", "snoopy","stop_sign","wheelchair"};
  ...
  */
    ...
    for(int i=0;i<59;i++){
        if (i==10){
            ...
            grafica[i] = ((double)contClasifiacionCorrectaGRAY/(double)clasificarGRAY.size())*double(59);
        }else if (i == 30){
            grafica[i] = (double)contClasifiacionCorrectaRGB;
        }else if (i == 35){
            grafica[i] = (double)clasificarRGB.size()-contClasifiacionCorrectaRGB;
        }else if (i == 40){
            grafica[i] = ((double)contClasifiacionCorrectaRGB/(double)clasificarRGB.size())*double(59);
        }
		/**
		else if (i == 50){
